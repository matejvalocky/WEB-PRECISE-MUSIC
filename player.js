


(function IIFE() {
  const list = [
    {
      // url: "./download/mp3/Drum-and-Bass-2023-Mix-no5-by-Precise-Music-(mp3-192).mp3",
      url: "https://drive.google.com/file/d/1K2BKYwfyDt7YYPO4DG9YhoEyTTwJcV2a/view?usp=sharing",
      author: "Precise Music",
      title: "Drum and Bass 2023 Mix #5",
      tracks: " 001 | Grafix & Flowidus - Submarine<br>002 | Delta Heavy - No Gravity<br>003 | Jack Mirror - With You (ft. flowanastasia)<br>004 | Kanine - Lost and Found<br>005 | Metrik - Abyss<br>006 | ShockOne - Ready To Go (ft. Bright Sparks)<br>007 | Freaks & Geeks - Function<br>008 | Mandidextrous & Tanukichi - Higher<br>009 | MK & Dom Dolla - Rhyme Dust (Dimension Remix)<br>010 | Fred V - Faded Blue (ft. Hybrid Minds & Lottie Jones)<br>011 | Mountain - Pegasus<br>012 | David Guetta, Becky Hill & Ella Henderson - Crazy What Love Can Do (Grafix Remix)<br>013 | C:rcle - Oscillate (ft. Gid Sedgwick)<br>014 | Alesso & John Newman - Call Your Name (Andromedik Remix)<br>015 | Andromedik - To The Dark (ft. MC Stretch)<br>016 | Dimension - DJ Turn It Up<br>017 | Dimension - Generator<br>018 | Dimension - Offender<br>019 | Kanine - Get Down<br>020 | Lee Mvtthews - Dice (ft. Rachel Leo)<br>021 | Metrik x ÆON:MODE - Awake<br>022 | S.P.Y - Complicated <br>023 | SIREN - Dark Nights (ft. Stuart Rowe)<br>024 | Solomon France - Sweet Kicks<br>025 | Supercurve - Frequency<br>026 | Tall Order - Underground<br>027 | Tantrum Desire - Bring It<br>028 | Technimatic - Sunburst<br>029 | Kove - So Much Love<br>030 | Natty Lou - Alpenglow (ft. Lottie Jones)<br>031 | Culture Shock - Alone (ft. Gracie Van Brunt)<br>032 | Junk Mail & Bugwell - Stronger<br>033 | Justin Hawkes - Heartleap<br>034 | Matrix & Futurebound & Synth System - Return To You (ft. Anna Simone)<br>035 | Metrik - Immortal<br>036 | Netsky & Hybrid Minds - Let Me Hold You<br>037 | Delta Heavy - Get Down Tonight (ft. Hayley May)<br>038 | Freaks & Geeks - Oxygen<br>039 | REAPER & Calivania - BLACK FIRES<br>040 | High Contrast - Can't Give You Up<br>041 | 1991 - Eternity (ft. Henry Dell)<br>042 | Ekko & Sidetrack - The Light (ft. Ruth Royall)<br>043 | Futurebound - Fall (ft. Dolly Rae)<br>044 | Krakota - Wanna Be Loved<br>045 | Metrik - We Are The Energy<br>046 | Sub Focus & Dimension - Ready To Fly<br>047 | Synergy - White Line<br>048 | Tisoki & Oliverse - Dream About You<br>049 | LOWES - Game of Love (Friction Remix)<br>050 | ShockOne - Heart On Fire (ft. Pauline Herr)<br>051 | Andromedik - With You<br>052 | Flowidus - Save Us (ft. Lauren L'aimant)<br>053 | Kings Of The Rollers - So Lost<br>054 | REPAIR - Obsessed<br>055 | Synergy - Freedom (ft. What So Not)<br>056 | D.O.D - So Much In Love (Sub Focus Remix)<br>057 | Lexurus - The Funk<br>058 | Lost Frequencies - The Feeling (Lost Frequencies & Andromedik Deluxe Mix)<br>059 | Bensley & Justin Hawkes - Don’t Be Scared<br>060 | Deep Notion - I Begged You<br>061 | Justin Hawkes - Better Than Gold (ft. Andrew Hellier) <br>062 | Delta Heavy x Jem Cooke - Heaven<br>063 | Friction & Poppy Baskcomb - The Answer<br>064 | Metrik - Fall To The Dust<br>065 | Wilkinson - Infinity (ft. ILIRA, iiola & Tom Cane)<br>",
      cover: "./images/covers/no5-mix-2023.jpg",
      youtube: "https://youtu.be/zeu43OWK1jc",
      spotify: "", 
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-2023-mix-no5-by-precise-music-mp3-192"
    },
    {
      url: "./download/mp3/Drum-and-Bass-2023-Mix-no4-by-Precise-Music-(mp3-192).mp3",
      author: "Precise Music",
      title: "Drum and Bass 2023 Mix #4",
      tracks:"001 | Friction & JP Cooper - Dancing<br>002 | Dawn Wall - Legends<br>003 | Fliwo - Yehay<br>004 | Dawn Wall - Movies<br>005 | Leks - Vices<br>006 | Quadrant & Iris ft. Ill Truth - Is This Now<br>007 | Riya, Collette Warren & L Side - All For Something<br>008 | Monty - Ferns<br>009 | Switch Disco, Robert Miles - REACT<br>010 | ZeroZero - Bring The Heat<br>011 | Document One - Jazz Club<br>012 | Enei & Kasra - Take A Dive<br>013 | Foreign Concept - Mob Justice<br>014 | Monrroe Ft. Sydney - As I Fall<br>015 | Alix Perez & Eprom - Caligo (Original Mix)<br>016 | Annix - Whatever U Did<br>017 | Dawn Wall - Dead Priest<br>018 | DNMO x Wolfy Lights - Bombalaya<br>019 | Murdock & Ruth Royall - Cola<br>020 | Pola & Bryson & Emily Makis - Phoneline<br>021 | DJ Marky - Changing Moods<br>022 | Pola & Bryson - Tell You What I Did<br>023 | Fred V - Dive<br>024 | Fred V - Homesick<br>025 | Solomon France - On The Weekend<br>026 | Sub Focus - I Found You<br>027 | Zero T & Onj - Games You Play<br>" , 
      cover: "./images/covers/no4-mix-2023.jpg",
      youtube: "https://www.youtube.com/watch?v=fxKwoyLZEE4",
      spotify: "https://open.spotify.com/playlist/41ViIW9mhedFhrCXnk4Tn4?si=4127248f8273425a", 
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-2023-mix-no4-by-precise-music-192-mp3"
    },
    {
      url: "./download/mp3/Drum-and-Bass-2023-Mix-no3-by-Precise-Music-(mp3-192).mp3",
      author: "Precise Music",
      title: "Drum and Bass 2023 Mix #3",
      tracks: "001 - BCee & Charlotte Haining - Is It Real?<br>002 - EDAN_BERROW - Get On Down<br>003 - Untrue - The Secret Pub<br>004 - The Creatures - Keep Me Close<br>005 - KALOA - Enso<br>006 - gyrofield - Tech Flex VIP<br>007 - Hiraeth & Drumslave - Bullseye<br>008 - Hystatus - Umph<br>009 - Macca & Cnof - Rattle Dub<br>010 - IAMDOOMED - Bionic<br>011 - Dunk & Black Opps - Rolling Golden<br>012 - Formula - Switch Blade<br>013 - Resslek - Zoom In<br>014 - Resslek - Undgrnd<br>015 - Body Model - Darkest Magic<br>016 - Body Model - Test Pilot<br>017 - Alpha Rosa - Whatever You Do<br>018 - Unglued x Whiney x Lens - If You Like That<br>019 - Nectax - What Lies Ahead<br>020 - Azifm & Scout - Think About Me<br>", 
      cover: "./images/covers/no3-mix-2023.jpg",
      youtube: "https://youtu.be/CMqSBgTzy8U",
      spotify: "", 
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-2023-mix-no3-by-precise-music-192-mp3"
    },
    {
      url: "./download/mp3/Drum-and-Bass-2023-Mix-no2-by-Precise-Music-(mp3-192).mp3",
      author: "Precise Music",
      title: "Drum and Bass 2023 Mix #2",
      tracks: "001 | Break - Bury a Sound<br>002 | Xyde - Anachronistic<br>003 | DLR & Total Science - Back Track<br>004 | Bredren - Inferno (ft. T-Man)<br>005 | Kyrist & Revaux - Shinto<br>006 | SubMarine - Back Then<br>007 | Duskee & Sustance - Scum<br>008 | Exult & Silloh - From The Trunk<br>009 | K Motionz - Back Trackin (ft. Comma Dee) <br>010 | Molecular - Saga (Willers Remix)<br>011 | clÜ - Mobsta<br>012 | Sustance Ft. Strategy - Undercurrent<br>013 | DLR & Script - It Wasn't A Dream<br>014 | Ill Truth & Bluejay - Street Art<br>015 | Ill Truth - Flex<br>016 | Inja x ArchiTek - Atom Splitter<br>017 | L0G1N & Willers - Anti-Aircraft Seagulls<br>018 | Operate & Rizzle - Mindset (ft. Rider Shafique)<br>019 | Ninive - Escort<br>020 | Operate & Rizzle - Azula<br>021 | Azifm - All Crew (ft. Hound)<br>022 | Kasra - I Don't Know What The Future Brings<br>023 | Turno, tominthechamber & Haribo - Triad<br>",
      cover: "./images/covers/no2-mix-2023.jpg",
      youtube: "https://youtu.be/i3iriRPOal4",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-2023-mix-2-by-precise-music"
    },
    { 
        
        url: "./download/mp3/Drum-and-Bass-2023-Mix-no1-by-Precise-Music-(mp3-192).mp3",
        author: "Precise Music",
        title: "Drum and Bass 2023 Mix #1",
        tracks: "0:00:00 | Black Barrel - Whatah Bang<br>0:03:17 | Dopplershift - Cradle of Humankind<br>0:07:39 | Akov - 'No More'<br>0:10:12 | Kyrist - Wreckage<br>0:13:50 | Black Barrel - Somebody Serious<br>0:15:18 | Break - Wait For You (ft. Lorna King)<br>0:18:58 | Vodkah - Severance<br>0:23:01 | Body Model - Test Pilot<br>0:25:35 | Noisia - Crank (Kasra Remix)<br>0:27:48 | Rueben - Spyware<br>0:32:13 | Trex & Eljay - God Damn Sound (Molecular Remix)<br>0:33:41 | Calyx - Feel The Sway<br>0:36:14 | Resnek - Conspiracy Theory<br>0:39:12 | EastColors - Toys<br>0:42:31 | MC Fava - Fly ft. Keeno<br>0:46:11 | Revaux - Follow Me<br>0:48:46 | Willers - Risky<br>0:51:20 | ZeroZero - A Bad Trip<br>0:54:17 | Zombie Cats & Waeys - Left Nothing<br>0:58:20 | Walk:r - Contour<br>",
        cover: "./images/covers/no1-mix-2023.jpg",
        youtube: "https://youtu.be/OMK27WBoH5g",
        spotify: "",
        housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-2023-mix-1-by-precise-music"
    },
    {
        
        url: "./download/mp3/DRUM-AND-BASS-DECEMBER-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
        author: "Precise Music",
        title: "Drum and Bass December Mix 2022",
        cover: "./images/covers/december-2022.jpg",
        tracks: "0:00:00 | Origin8a & Propa ft. Benny Page - Harmony<br>0:02:12 | Survey - Far Gone<br>0:04:03 | Document One ft. Rider Shafique - Pounds nor Pence<br>0:06:37 | Ill Truth & Black Barrel - Zoo<br>0:08:17 | Molecular - Acid Funk<br>0:10:19 | Rockwell - Peak (ft. Novelist)<br>0:12:31 | Derrick & Tonika - Strong Pipe (Original)<br>0:15:05 | Break - Better Be Sure<br>0:16:56 | Enei - Falling<br>0:19:08 | DLR - Like This<br>0:21:43 | Atlantic Connection - Situations (DLR Remix)<br>0:24:17 | Koherent & Edit - Dis One<br>0:26:07 | Screamarts - Funk Me Up<br>0:27:36 | Waeys - Patched<br>0:30:11 | Molecular - Who Goes There (feat. MC Gusto)<br>0:32:01 | Molecular - Figure It Out<br>0:33:51 | Hadley & Waeys - Who You Are<br>0:35:41 | Whiney - Talisman<br>0:37:54 | Submotive & Vektah - Groove Control<br>",
        youtube: "https://youtu.be/of9JS_25xXk",
        spotify: "",
        housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-december-mix-2022-mixed-by-precise-music"
    },
    {
        
        url: "./download/mp3/DRUM-AND-BASS-NOVEMBER-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
        author: "Precise Music",
        title: "Drum and Bass November Mix 2022",
        cover: "./images/covers/november-2022.jpg",
        tracks: "0:00:00 | Calibre - Even If - Original Mix<br>0:06:38 | GLXY - Something Good<br>0:09:24 | Lenzman - Wet Like Water<br>0:11:59 | Macca - Special Lady<br>0:14:33 | Workforce - For What Sustains<br>0:17:52 | Zero T - Go<br>0:20:04 | Artsea - Discovery<br>0:23:00 | Hugh Hardie - Blush<br>0:25:13 | Lenzman - Playing It Off<br>0:27:47 | Pyxis - Elias<br>0:30:44 | GLXY - Dreamers<br>0:34:24 | Skuff - Gotta Mean Something<br>0:37:43 | Visages - Love Conspiration<br>0:39:33 | Break - Natural Way (ft. MC Fats)<br>0:41:46 | Subtle Element - Crinkled<br>0:45:49 | Waeys - Mapper (Rift Remix)<br>0:49:09 | Zero T & Steo - StopStart<br>0:51:42 | Waeys - Unfinished Business<br>0:54:17 | LSB - Ghost Dub - Spearhead Records<br>0:56:29 | Non Fiction (Main Mix)<br>0:58:41 | Random Movement - Slinkystink<br>",
        youtube: "https://youtu.be/cgqnUqmwE5c",
        spotify: "",
        housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-november-mix-2022-mixed-by-precise-music"
    },
    {
        
        url: "./download/mp3/DRUM-AND-BASS-OCTOBER-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
        author: "Precise Music",
        title: "Drum and Bass October Mix 2022",
        cover: "./images/covers/october-2022.jpg",
        tracks: "0:00:00 │ BCee - Vicarage Road<br>0:05:09 │ Dan Guidance - Inner Groove<br>0:07:43 │ Friction - Weed & Wine (ft. A Little Sound)<br>0:09:55 │ Hiraeth - Our Love<br>0:13:14 │ In:Most - Silhouette (feat. Visionobi) (Flava D Remix)<br>0:15:27 │ Jack Boston - This Time (ft. Tenisha Edwards)<br>0:19:51 │ Break - Something Like That<br>0:22:48 │ Flava D - Alive<br>0:25:22 │ Justin Hawkes - Dreambend<br>0:28:19 │ Rueben & PHI NIX - Memories<br>0:30:53 │ Unglued x Degs - Nothing To Lose<br>0:32:43 │ Waeys - The Rush<br>0:35:18 │ ZeroZero & Linja - Gobi<br>0:38:37 │ ZeroZero - Tracker<br>0:40:05 │ Beeson & Dynamite MC - 'Til 4am<br>0:42:17 │ Bredren - Betta (feat. S.P.Y)<br>0:44:30 │ Creatures & Philth - Love's Ghost<br>0:48:10 │ DJ Hybrid - Don't Get It Confused<br>0:50:45 │ Nymfo - Sick With It<br>0:54:25 │ Fred V x Audioscribe - Lose Yourself Again<br>0:57:00 │ Netsky - The Lotus Symphony<br>1:00:19 │ Raffa FL - Ritmo (Document One Remix)<br>1:02:42 │ Rueben - Apathy<br>1:05:38 │ Philth & Trex - Cloud Riddim<br>1:08:57 │ Waeys & Unglued - Take 2007<br>",
        youtube: "https://youtu.be/ZEWtux4kd30",
        spotify: "",
        housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-october-mix-2022-mixed-by-precise-music"
    },
    {
        
        url: "./download/mp3/DRUM-AND-BASS-SEPTEMBER-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
        author: "Precise Music",
        title: "Drum and Bass September Mix 2022",
        cover: "./images/covers/september-2022.jpg",
        tracks: "0:00:00 │ Break - If I Could <br> 0:04:03 │ Total Science - Walk The Same Lines (ft. Riya) <br> 0:06:59 │ Logistics - Take Me To Another World (ft. Stella Attar) <br> 0:10:18 │ Metrik - Chasing Sunrise (feat. Elisabeth Troy) <br> 0:12:30 │ Technimatic - Night Vision - VIP Mix <br>0:13:58 │ Technimatic - Transference <br>0:15:27 │ Whiney - Moondance <br>0:18:45 │ Fred V & Grafix - Comb Funk <br>0:20:57 │ High Contrast - Questions (ft. Boy Matthews) <br>0:23:21 │ Hugh Hardie - Raindrops (ft. Cimone) <br>0:25:11 │ Ownglow - Not Like Me <br>0:28:13 │ Treex - Troubled Minds - Original Mix <br>0:30:48 │ Hugh Hardie - City Soul (feat. Silence Groove) <br>0:33:45 │ Hugh Hardie - Light It Up <br> 0:35:13 │ Hugh Hardie - She Moves (feat. GLXY, 3-Card & Zoë Phillips) <br>0:37:25 │ LSB - Tumult <br>0:40:44 │ Ownglow - Tension <br>0:41:50 │ TC - Hold On <br>0:44:24 │ Holy Goof - Tell Me (ft. Paige Cavell) <br>   0:45:31 │ Bastion - Radio <br> 0:46:37 │ Leniz - Mourning Tears <br>0:49:33 │ Technimatic x LSB - Sight Lines <br>0:53:14 │ Alix Perez & Bou - Back To Basics (ft. Trigga) <br>0:56:33 │ Trex - Do Nothing <br>0:57:39 │ Rido - Faith <br>0:59:07 │ Elipsa - Survival <br>1:00:57 │ Enei - Show Me (ft. Odddi) <br>",
        youtube: "https://youtu.be/ZKtu9xorkrg",
        spotify: "https://open.spotify.com/playlist/7c9Waq2toatp2DpAa1RBBf?si=ece70baf514546c5",
        housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-september-mix-2022-mixed-by-precise-music"
    },
    {
        
        url:"./download/mp3/DRUM-AND-BASS-AUGUST-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
        author:   "Precise Music",
        title:    "Drum and Bass August Mix 2022",
        cover:    "./images/covers/august-2022.jpg",
        tracks:   "0:00:00 | Forthcoming - Dreams Do Come True<br>0:04:48 | Hugh Hardie - Said & Done (feat. DJ Marky & Cimone)<br>0:08:04 | Lenzman - Golden Age Full Vocal Mix feat Dan Stezo & Steo<br>0:11:47 | Mage - Lucky Charm<br>0:15:48 | Mind Vortex - Now It's Time<br>0:19:50 | Nu:Logic - Everlasting Days (Ft. Lifford)<br>0:23:31 | Paul SG - Stay Classy - feat. T.R.A.C<br>0:28:20 | Phaction ft. Leo Wood - I Have You (Ill Truth Remix)<br>0:32:02 | Radicall - Polski Jazz (Original Mix)<br>0:35:40 | Random Movement - Goblin Jazz Banquet<br>0:39:23 | Satl - Squeeze<br>0:43:03 | Subtension & Dephzac - Tomb Driver<br>0:46:42 | Unknown Artist - Mango<br>0:50:42 | Wesland - Meeting Friends<br>0:55:32 | Blade & Jrumhand - Miles Groove<br>0:59:55 | Command Strange - Blues<br>1:04:00 | Danny Byrd x D Double E – Selecta<br>1:06:01 | DJ Limited - Summer Vibes feat Lizzy Stringer<br>1:10:03 | Dossa & Locuzzed - Crush<br>1:13:44 | Frazah - Sambaso<br>1:16:41 | Funkware - Get Up - Original Mix<br>1:20:22 | Funkware - Never Know How Much<br>1:24:25 | Keeno - Equinox<br>1:27:55 | Lenzman - In My Mind (feat. IAMDDB)<br>1:31:35 | Makoto x Hugh Hardie - Bluebird<br>1:34:54 | Phaeny - Blaha (Original Mix)<br>1:39:30 | Picota & Kumbh - 1960<br>1:43:11 | Redeyes - Carry Me Home<br>1:46:51 | Shy FX - Call Me ft. Maverick Sabre<br>1:49:25 | The Invaderz - Love Vibrations (feat. DJ Marky)<br>1:54:13 | Dabs - Buntlack (Kyrist Remix)    <br>1:57:31 | Nemy & Shire - Victims<br>1:59:46 | Embr - Set The Tone<br>2:03:04 | BassBrothers - Love You So (Original Mix)<br>2:06:46 | DRS - Star Voyager (feat. S.P.Y. & Jenna G)<br>2:10:04 | Funkware - Guess What (Original Mix)<br>2:15:13 | InnaSelf - Jazz Fifty Nine (Original Mix)<br>2:19:15 | Paul SG - Verrazano<br>2:22:35 | Simple Souls - Day & Night<br>2:26:16 | Theoretical - Release the Energy<br>2:29:55 | Bally & Boom - Yeah<br>2:32:52 | Mountain - I’m Free (Hugh Hardie Remix)<br>2:35:27 | Mr Joseph x Illmatika - Her Name Is feat. Sabrina Carr<br>2:39:52 | Paul SG & DJ Marky - Cafe International<br>2:43:10 | Random Movement - Feels Like It Used To<br>2:47:38 | Simple Souls - Flying<br>2:51:19 | Nelver - Rotation (Original Mix)<br>2:56:50 | Tidal - Jazz Tonight (Original Mix)<br>2:59:24 | Bally & Boom - Let It Whip<br>3:02:55 | Eugenics Eight - She Can Wait Forever (Original Mix)<br>",
        youtube:  "https://youtu.be/2QvT9zR4qoY?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
        spotify:  "",
        housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-august-mix-2022-mixed-by-precise-music"

    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-JULY-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass July Mix 2022",
      cover: "./images/covers/july-2022.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/pIVUYeddO-Q?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "https://open.spotify.com/playlist/53HghvfrRU5bxJ4dd2cduN",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-july-mix-2022-mixed-by-precise-music"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-JUNE-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass June Mix 2022",
      cover: "./images/covers/june-2022.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/lMQyJQ8uUks?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "https://open.spotify.com/playlist/5jINvKju5GptlMuJedV7zV",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-june-mix-2022-mixed-by-precise-music"
    },  
    {
      
      url:"./download/mp3/DRUM-AND-BASS-MAY-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass May Mix 2022",
      cover: "./images/covers/may-2022.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/cKrzy9FKprY?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "https://open.spotify.com/playlist/0iMVgpgnBvjfYQ9gbZ0LYf",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-may-mix-2022-mixed-by-precise-m"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-APRIL-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass April Mix 2022",
      cover: "./images/covers/april-2022.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/WHc76yVmSSI?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: ""
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-MARCH-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass March Mix 2022",
      cover: "./images/covers/march-2022.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/9v7sjVd6jOc?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-march-mix-2022-mixed-by-precise"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-FEBRUARY-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass February Mix 2022",
      cover: "./images/covers/february-2022.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/L1cpwv_vDhQ?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-february-mix-2022-mixed-by-prec"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-JANUARY-MIX-2-2022-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass January Mix #2 2022",
      cover: "./images/covers/january2-2022.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/z62zE5EpCkY?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-january-mix-2-2022-mixed-by-pre"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-JANUARY-MIX-2022-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass January Mix #1 2022",
      cover: "./images/covers/january-2022.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/JaNK75j0tA4?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-january-mix-2022-mixed-by-preci"
    },
    {
      
      url: "./download/mp3/DRUM-AND-BASS-DECEMBER-MIX-2-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass December Mix #2 2021",
      cover: "./images/covers/december-2-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/9sO2oKi-DRE?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-december-mix-2-2021-mixed-by-pr"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-DECEMBER-MIX-1-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass December Mix #1 2021",
      cover: "./images/covers/december-1-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/L-f8MB2Xc74?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-december-mix-1-2021-mixed-by-pr"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-NOVEMBER-MIX-2-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass November Mix #2 2021",
      cover: "./images/covers/november-2-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/p5HkuuWNaqc?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-november-mix-2-2021-mixed-by-pr"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-NOVEMBER-MIX-1-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass November Mix #1 2021",
      cover: "./images/covers/november-1-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/ZUuQvXyMbV0?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-november-mix-1-2021-mixed-by-pr"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-OCTOBER-MIX-2-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass October Mix #2 2021",
      cover: "./images/covers/october-2-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/N3xl1QAeRj0?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-october-mix-2-2021-mixed-by-pre"
    },
    {
      
      url:"./download/mp3/DRUM-AND-BASS-OCTOBER-MIX-1-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass October Mix #1 2021",
      cover: "./images/covers/october-1-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://youtu.be/Xfg1qidEkk4?list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-october-mix-1-2021-mixed-by-pre"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-SEPTEMBER-MIX-2-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass September #2 Mix 2021",
      cover: "./images/covers/september-2-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=-Qn-pp1vxjo&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=23&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-september-mix-2-2021-mixed-by-p"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-SEPTEMBER-MIX-1-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass September #1 Mix 2021",
      cover: "./images/covers/september-1-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=NpKtWaqa07o&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=24&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-september-mix-2021-mixed-by-pre"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-AUGUST-MIX-2-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass August #2 Mix 2021",
      cover: "./images/covers/august-2-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=4roXvJ_qj78&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=25&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-august-mix-2021-2-by-precise-mu"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-AUGUST-MIX-1-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass August #1 Mix 2021",
      cover: "./images/covers/august-1-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=4roXvJ_qj78&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=25&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-august-mix-2021-by-precise-musi"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-JULY-MIX-2-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass JULY #2 Mix 2021",
      cover: "./images/covers/july-2-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=6JAXfoxUqyI&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=27&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-july-mix-2021-2-by-precise-musi"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-JULY-MIX-1-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass JULY #1 Mix 2021",
      cover: "./images/covers/july-1-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=0nw61rBHT0Q&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=28&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-july-mix-2021-mixed-by-precise"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-JUNE-MIX-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass June Mix 2021",
      cover: "./images/covers/june-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=T_-xHgutgYg&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=29&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-june-mix-2021-mixed-by-precise"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-MAY-MIX-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass May Mix 2021",
      cover: "./images/covers/may-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=IX_N0PebSmQ&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=30&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-may-mix-2021-mixed-by-precise-m"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-APRIL-MIX-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass April Mix 2021",
      cover: "./images/covers/april-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=I8yAWBP8wjs&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=31&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-april-mix-2021-mixed-by-precise"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-MARCH-MIX-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass March Mix 2021",
      cover: "./images/covers/march-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=fQjeo3pn7Qg&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=32&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-march-mix-2021-mixed-by-precise"
    },
    {
      url:"./download/mp3/DRUM-AND-BASS-JANUARY-MIX-2021-MIXED-BY-PRECISE-MUSIC.mp3",
      author: "Precise Music",
      title: "Drum and Bass January Mix 2021",
      cover: "./images/covers/january-2021.jpg",
      tracks: "TRACKLIST SOON",
      youtube: "https://www.youtube.com/watch?v=yGS6uX8XojM&list=PL8sueKk45oWuIUD12VpGmtRgxfJqGDW7X&index=33&pp=iAQB8AUB",
      spotify: "",
      housemixes: "https://www.house-mixes.com/profile/PreciseMusic/drum-and-bass-january-mix-2021-mixed-by-preci"
    },
    // {
    //   url:"./download/mp3/DRUM-AND-BASS-XXXXXXXX-MIX-2020-MIXED-BY-PRECISE-MUSIC.mp3",
    //   author: "Precise Music",
    //   title: "Drum and Bass xxxxxxx Mix 2020",
    //   cover: "./images/covers/xxxxxxxx.jpg",
    //   tracks: "TRACKLIST SOON",
    //   youtube: "",
    //   spotify: "",
    //   housemixes: ""
    // },
];



    

    


    




  
    let currentId = 0;
    let isPlaying = false;
    let isLoop = true;
    let isShuffle = false;
    let currentAudio = "music1";
    let timer = null;
    let loopOne = false;
  
    const currentTimeIndicator = document.querySelector(".music-time__current");
    const leftTimeIndicator = document.querySelector(".music-time__last");
    const progressBar = document.getElementById("length");
    const playBtn = document.querySelector(".play");
    const cover = document.querySelector(".cover");
    const title = document.querySelector(".music-player__title");
    const author = document.querySelector(".music-player__author");
// PLAYLIST
    const playlist = document.querySelector(".playlist");
// PLAYLIST
    const loopBtn = document.getElementById("loop");
    const shuffleBtn = document.getElementById("shuffle");
    const forwardBtn = document.getElementById("forward");
    const backwardBtn = document.getElementById("backward");
    const prevBtn = document.getElementById("prev");
    const nextBtn = document.getElementById("next");
    const progressDiv = document.getElementById("progress");
  
    function play(e) {
      if (!isPlaying) {
        // console.log('play');
        e.target.src =
          "https://snowleo208.github.io/100-Days-of-Code/7.%20Music%20Player/img/pause.svg";
        e.target.alt = "Pause";
        isPlaying = true;
        document.getElementById(currentAudio).play();
        showTime();
      } else {
        // console.log('pause');
        e.target.src =
          "https://snowleo208.github.io/100-Days-of-Code/7.%20Music%20Player/img/play.svg";
        e.target.alt = "Play";
        document.getElementById(currentAudio).pause();
        isPlaying = false;
        clearInterval(timer);
      }
    }
  
    function changeBar() {
      const audio = document.getElementById(currentAudio);
      const percentage = (audio.currentTime / audio.duration).toFixed(3);
      progressBar.style.transition = "";
      // console.log(audio.currentTime);
  
      //set current time
      const minute = Math.floor(audio.currentTime / 60);
      const second = Math.floor(audio.currentTime % 60);
      const leftTime = audio.duration - audio.currentTime;
      currentTimeIndicator.innerHTML =
        ("0" + minute).substr(-2) + ":" + ("0" + second).substr(-2);
  
      //set left time
      const leftMinute = Math.floor(leftTime / 60);
      const leftSecond = Math.floor(leftTime % 60);
  
      leftTimeIndicator.innerHTML =
        ("0" + leftMinute).substr(-2) + ":" + ("0" + leftSecond).substr(-2);
  
      //set time bar
      progressBar.style.width = percentage * 100 + "%";
    }
  
    function showTime() {
      timer = setInterval(() => changeBar(), 500);
    }
  
    function nextMusic(mode) {
      playBtn.src =
        "https://snowleo208.github.io/100-Days-of-Code/7.%20Music%20Player/img/play.svg";
      playBtn.alt = "Play";
      document.getElementById(currentAudio).pause();
      isPlaying = false;
      clearInterval(timer);
  
      if (mode === "next") {
        currentId = currentId + 1 > list.length - 1 ? 0 : currentId + 1;
        init();
      } else {
        currentId = currentId - 1 < 0 ? list.length - 1 : currentId - 1;
        init();
      }
    }
  
    function shuffle(e) {
      isShuffle = !isShuffle;
      if (isShuffle) {
        e.target.parentNode.classList.add("is-loop");
      } else {
        e.target.parentNode.classList.remove("is-loop");
      }
    }
  
    function backward() {
      const audio = document.getElementById(currentAudio);
      audio.currentTime -= 5;
      if (!isPlaying) {
        changeBar();
      }
    }
  
    function forward() {
      const audio = document.getElementById(currentAudio);
      audio.currentTime += 5;
      if (!isPlaying) {
        changeBar();
      }
    }
  
    function stopMusic() {
      playBtn.src =
        "https://snowleo208.github.io/100-Days-of-Code/7.%20Music%20Player/img/play.svg";
      playBtn.alt = "Play";
      isPlaying = false;
    }
  
    function goToNextMusic() {
      let newId = currentId;
      while (isShuffle && !loopOne && newId === currentId) {
        newId = Math.floor(Math.random() * Math.floor(list.length - 1));
      }
  
      if (!isShuffle && !loopOne) {
        currentId = currentId + 1 > list.length - 1 ? 0 : currentId + 1;
      }
      if (!isShuffle && loopOne) {
        currentId = currentId;
      }
  
      if (isShuffle) {
        currentId = newId;
      }
      init();
      document.getElementById(currentAudio).play();
    }
  
    function loop(e) {
      const audio = document.getElementById(currentAudio);
  
      if (!isLoop && !loopOne) {
        isLoop = true;
        loopOne = false;
        // console.log('is loop');
        e.target.parentNode.classList.add("is-loop");
        e.target.src =
          "https://snowleo208.github.io/100-Days-of-Code/7.%20Music%20Player/img/loop.svg";
        audio.loop = false;
        audio.onended = (e) => goToNextMusic();
        console.log(isLoop, loopOne);
      } else if (isLoop && !loopOne) {
        // console.log('is loop one');
        isLoop = true;
        loopOne = true;
        e.target.parentNode.classList.add("is-loop");
        e.target.src =
          "https://snowleo208.github.io/100-Days-of-Code/7.%20Music%20Player/img/loopone.svg";
        audio.loop = true;
        audio.onended = (e) => goToNextMusic();
        console.log(isLoop, loopOne);
      } else {
        // console.log('not loop');
        isLoop = false;
        loopOne = false;
        e.target.parentNode.classList.remove("is-loop");
        e.target.src =
          "https://snowleo208.github.io/100-Days-of-Code/7.%20Music%20Player/img/loop.svg";
        audio.loop = false;
        audio.onended = (e) => stopMusic();
        console.log(isLoop, loopOne);
      }
    }
  
    function progress(e) {
      const audio = document.getElementById(currentAudio);
      //get current position and minus progress bar's x position to get current position in progress bar
      const pos =
        (e.pageX - progressDiv.getClientRects()[0].x) /
        progressDiv.getClientRects()[0].width;
      audio.currentTime = pos * audio.duration;
      changeBar();
    }
  
    function init() {
      //reset music duration and setup audio
      const audio =
        document.getElementById(currentAudio) === null
          ? new Audio()
          : document.getElementById(currentAudio);
      audio.src = list[currentId].url;
      audio.id = currentAudio;
      document.getElementById(currentAudio) === null
        ? document.body.appendChild(audio)
        : "";
  
      progressBar.style.transition = "none";
      progressBar.style.width = "0%";
      document.getElementById(currentAudio).currentTime = 0;
  
      title.innerHTML = list[currentId].title;
      author.innerHTML = list[currentId].author;






// SHOW PLAYLIST
      playlist.innerHTML = list[currentId].tracks;
// SHOW PLAYLIST


// ADD DOWNLOAD LINK 
      const downloadLink = document.querySelector("#downloadLink");
      const mixUrl = list[currentId].url;
      downloadLink.setAttribute("href", mixUrl);
// ADD DOWNLOAD LINK 



// ADD YOUTUBE LINK
      const youtubeLink = document.querySelector("#youtubeLink");
      const youtubeUrl = list[currentId].youtube;
      youtubeLink.setAttribute("href", youtubeUrl)
// ADD YOUTUBE LINK




//ADD SPOTIFY LINK
      const spotifyLink = document.querySelector("#spotifyLink");
      const spotifyUrl = list[currentId].spotify;
      spotifyLink.setAttribute("href", spotifyUrl)
//ADD SPOTIFY LINK


//ADD HOUSE-MIXES LINK
      const housemixesLink = document.querySelector("#housemixesLink");
      const housemixesUrl = list[currentId].housemixes;
      housemixesLink.setAttribute("href", housemixesUrl)
//ADD HOUSE-MIXES LINK



    // VYPISANIE CIEST K OBRAZKOM COVEROV DO KONZOLE, POZITIE CYKLU FOR A PRIDANIE OBRAZKU DO SEKCIE S ID MIXES






    



    window.addEventListener('load', function() {
      for (let i = 0; i < list.length; i++) {
        if (list[i].hasOwnProperty("cover")) {
            // console.log (list[i].cover)
  
            // ODCHYTENIE SI HTML TAGU S ID mixes
        let mixCover = document.querySelector("#mixes");
     
        // vytvorte prvok <img> a nastavte jeho atribút src
                const image = document.createElement('img');
                image.src = (list[i].cover);
        
        // pridajte <img> ako potomka <p>
                mixCover.appendChild(image);  

        // vypisanie prislušneho indexu z pola objektov s nazvom "list"
                image.addEventListener('click', () => {

                  console.log(i);              
                  currentId = i;
                  init();
                });

                
                
        }
      };
    });

    







      


      

      

      

      

      
      
      


      

      











      

      
      
      cover.src = list[currentId].cover;
  
      //set current time
      audio.addEventListener("loadedmetadata", function () {
        const leftMinute = Math.floor(audio.duration / 60);
        const leftSecond = Math.floor(audio.duration % 60);
        currentTimeIndicator.innerHTML = "00:00";
        leftTimeIndicator.innerHTML =
          ("0" + leftMinute).substr(-2) + ":" + ("0" + leftSecond).substr(-2);
        progressBar.style.transition = "";
      });
  
      //set loop
      document.getElementById(currentAudio).onended = (e) => goToNextMusic(e);
    }
  
    playBtn.addEventListener("click", play);
    loopBtn.addEventListener("click", loop);
  
    shuffleBtn.addEventListener("click", shuffle);
    forwardBtn.addEventListener("click", forward);
    backwardBtn.addEventListener("click", backward);
  
    prevBtn.addEventListener("click", (e) => nextMusic("prev"));
    nextBtn.addEventListener("click", (e) => nextMusic("next"));
    progressDiv.addEventListener("click", (e) => {
      progress(e);
    });
  
    init();
  })();
  



